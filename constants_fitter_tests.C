...
int frame(){
  ...
  RDataFrame df("track_tree", {"/scratchnvme/alexe/MomentumBiases/track_params.root"});
  //RDataFrame df_cut("track_tree", "/scratchnvme/alexe/MomentumBiases/track_params_M_no_cut.root");

  auto red_chi2_edm = df.Histo2D({"red_chi2_edm", "edm vs red chi2", 50, *df.Min<double>("red_chi2"), *df.Max<double>("red_chi2"), 50, *df.Min<double>("edm"), *df.Max<double>("edm")},"red_chi2","edm");
  auto edm_low = df.Histo1D({"edm_low", "edm bin", 30, *df.Min<double>("edm"), *df.Max<double>("edm")},"edm");
  ...
  TH1D *param_mean_e = new TH1D("param_mean_e", "param_pull_mean e", 3, 0.0, 3.0);
  param_mean_e->SetCanExtend(TH1::kAllAxes);
  TH1D *param_width_e = new TH1D("param_width_e", "param_pull_width e", 3, 0.0, 3.0);
  param_width_e->SetCanExtend(TH1::kAllAxes);
  ...
  for(int i=0; i<=23; i++){ //TODO loop upper bound input by hand
    ...
    name = Form("pull_e%d",i);
    delete gROOT->FindObject(name.c_str());
    minVal = df.Min<double>(name.c_str());
    maxVal = df.Max<double>(name.c_str());
    temp = df.Histo1D({name.c_str(), "edm > 5.345e-4", 50, *minVal, *maxVal},name.c_str());
    param_mean_e->SetBinError(param_mean_e->Fill(name.c_str(), temp->GetMean()), temp->GetMeanError());
    param_width_e->SetBinError(param_width_e->Fill(name.c_str(),temp->GetStdDev()), temp->GetStdDevError());
  ...
  */
  ...
  param_mean_e->Fit("pol0");
  param_mean_e->SetStats(0);
  param_mean_e->SetLineColor(kBlue);

  param_mean_e->GetFunction("pol0")->SetLineColor(kBlue);
  leg4->AddEntry(param_mean_e, "no edm cut", "l");
  param_mean_e->Draw("");

  leg4->Draw("SAME");
  f.WriteObject(c6, "param_mean_e");

  TCanvas *c7 = new TCanvas("c7","c7",800,600);

  auto leg5 = new TLegend(0.68, 0.78, 0.90, 0.90);
  ...
  param_width_e->Fit("pol0");
  param_width_e->SetStats(0);
  param_width_e->SetLineColor(kBlue);

  param_width_e->GetFunction("pol0")->SetLineColor(kBlue);
  leg5->AddEntry(param_width_e, "no edm cut", "l");
  param_width_e->Draw("");

  leg5->Draw("SAME");
  f.WriteObject(c7, "param_width_e");
