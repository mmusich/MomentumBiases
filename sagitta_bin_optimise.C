...
int frame(){
	...
	for (int neg_pt_bin=1; neg_pt_bin<=nbinspt; neg_pt_bin++){
	  ...
	  if (nevents < 100.0){ // reject low stats
		...
		// epsilon diff_smear
		  ...
		  J_control(position_to_fill,2) = value; //J_control.col(2) is for alpha-1
	      ...
	      // ATTENTION: n_e_a_vector_control contains nu-1, epsilon, alpha-1
	      ...
	      nu_control->SetBinError(nu_control->Fill(name.c_str(), n_e_a_vector_control(0)+1), pow(V_nu_control(0,0),0.5)); // +1 to n_e_a_vector(0) to get nu 
	      ...
	      leg1->AddEntry((TObject*)0, leg_entry.c_str(), "");
	      leg_entry = "Fit #nu=" + to_string(n_e_a_vector_control(0)+1).substr(0, 6) + "#pm" + to_string(pow(V_nu_control(0,0),0.5)).substr(0, 6); // +1 to n_e_a_vector_control(0) to get nu 
	      leg1->AddEntry((TObject*)0, leg_entry.c_str(), "");
	      leg_entry = "Fit #alpha=" + to_string(n_e_a_vector_control(2)+1).substr(0, 6) + "#pm" + to_string(pow(V_alpha_control(0,0),0.5)).substr(0, 6); // +1 to n_e_a_vector_control(2) to get alpha
              leg1->AddEntry((TObject*)0, leg_entry.c_str(), "");
		...
		if ( find(good_indices_mll_diff.begin(), good_indices_mll_diff.end(), i) != good_indices_mll_diff.end() ){
		  ...
		  //	      pow(n_e_a_vector_control(0)*multi_hist_proj_diff_smear->GetBinErrorLow(i),2) + // need nu-1, which is n_e_a_vector_control(0)
		  ...
		  error = pow(
		            ...
		            pow(n_e_a_vector_control(2)*jac_control->GetBinErrorLow(i),2) + // need alpha-1, which is n_e_a_vector_control(2)
  ...
  std::cout<<"epsilon control: \n";
  epsilon_control->Fit("pol0");
