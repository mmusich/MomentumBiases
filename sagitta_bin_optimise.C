...
int frame(){
  ...
  int filled_bins_mll, position_to_fill, filled_bins_mll_diff, unchecked_fits=0, ndf_mc;
  string name, leg_entry;
  vector<double> fitresult;

  // Histograms for mll_diff distribution properties

  TH1D *mean_diff = new TH1D("mean_diff", "smear-gen mll mean", 3, 0, 3);
  mean_diff->SetCanExtend(TH1::kAllAxes);
  mean_diff->SetMarkerStyle(kPlus);
  mean_diff->GetXaxis()->SetTitle("Bin number");
  mean_diff->GetYaxis()->SetTitle("smear-gen mll mean [GeV]");

  TH1D *sigma_diff = new TH1D("sigma_smear", "smear-gen mll sigma", 3, 0, 3);
  sigma_diff->SetCanExtend(TH1::kAllAxes);
  sigma_diff->GetXaxis()->SetTitle("Bin number");
  sigma_diff->GetYaxis()->SetTitle("smear-gen mll sigma [GeV]");
  ...
  jac_inclusive->GetXaxis()->SetTitle("mll reco");
  ...
  jac_epsilon_inclusive->GetXaxis()->SetTitle("mll reco");
  ...
  TH1D *pull_epsilon_control3 = new TH1D("pull_epsilon_control3", " ((mean_yellow - mean_green) - epsilon) / error_epsilon (0.5< chi2/ndf green < 2) ", bins, limit_m, limit_p);
  ...
  total_nevents = multi_hist_proj_diff_smear->Integral(1,nbinsmll_diff);
  ...
  for (int pos_eta_bin=1; pos_eta_bin<=nbinseta; pos_eta_bin++){
	...
	for (int neg_pt_bin=1; neg_pt_bin<=nbinspt; neg_pt_bin++){
	  ...
	  // Require enough stats in mll_diff for sigma_MC fit
	  delete gROOT->FindObject("multi_data_histo_diff_smear_proj_4");
	  multi_hist_proj_diff_smear = mDh_diff_smear->Projection(4);
	  nevents = multi_hist_proj_diff_smear->Integral(1,nbinsmll_diff); 
	  
	  if (nevents < 300.0){ // reject low stats
	    empty_histos_count++;
	  } else {

	    // Require most of mll_smear Gaussian to be in fit window
	    ...
	    fitresult = fitHisto(multi_hist_proj_smear, 0, 8, 5);
	    ...
	    if (fitresult[4] < 0.75){ // reject small gaus integral 
	      ...
	      // Already projected diff_smear
	      //multi_hist_proj_diff_smear->SetName(name.c_str());
	      ...
	      // Fit diff_smear
              fitresult = fitHisto(multi_hist_proj_diff_smear, 1, 8, 2);
              if (fitresult[0] > -90.0){ mean_diff->SetBinError(mean_diff->Fill(name.c_str(), fitresult[0]), fitresult[2]); }
              if (fitresult[1] > -5.0){ sigma_diff->SetBinError(sigma_diff->Fill(name.c_str(), fitresult[1]), fitresult[3]); }
              ...
              chi2_mc = fitresult[5];
	      //-----------------------------------------------
              // ATTENTION it's a pretty dodgy way to get ndf
              ndf_mc = (4.0*sigma_mc/(10.0/nbinsmll_diff) - 3);
	      //-----------------------------------------------
	      ...
	      fitresult = fitHisto(multi_hist_proj_diff_smear_beta_val, 1, 5, 2);
	      ...
	      fitresult = fitHisto(multi_hist_proj_diff_smear_beta_val_easy, 1, 6, 2);
	      ...
	      //multi_hist_proj_diff_smear_plus_offset->SetLineColor(kRed);
              ...
              //fitresult = fitHisto(multi_hist_proj_diff_smear_plus_offset, 1, 2, 2);
	      ...
	      //--------------------------------------------------------------------------
              ...
              //fitresult = fitHisto(multi_hist_proj_diff_smear_minus_offset, 1, 4, 2);
	      ...
	      //fitresult = fitHisto(multi_hist_proj_smear_beta_val, 0, 5, 5);
		...
		if ( find(good_indices_mll_diff.begin(), good_indices_mll_diff.end(), i) != good_indices_mll_diff.end() ){
	      ...
	      fitresult = fitHisto(corrected_diff_smear, 1, 1, 2);
	      ...
	      if ( chi2_mc / ndf_mc < 2 && chi2_mc / ndf_mc > 0.5){
		...
		epsilon_test2->Fill(abs((mean_diff_smear_epsilon_val - mean_mc) / sigma_mc), (mean_diff_smear_epsilon_val - mean_mc - n_e_a_vector_control(1))/pow(V_epsilon_control(0,0),0.5));
	      ...
	      leg_entry = "Pull #varepsilon=" + to_string( (mean_diff_smear_epsilon_val - mean_mc - n_e_a_vector_control(1))/pow(V_epsilon_control(0,0),0.5) ).substr(0, 6) + "#chi^{2}/ndf green=" + to_string(chi2_mc/ndf_mc).substr(0, 6) ;
	      leg1->AddEntry((TObject*)0, leg_entry.c_str(), "");
              leg1->Draw("");
  ...
  f_control->WriteObject(mean_diff, "mean_diff_smear");

  sigma_diff->SetStats(0);
  sigma_diff->LabelsDeflate();
  f_control->WriteObject(sigma_diff, "sigma_diff_smear");
  ...
  fitresult = fitHisto(pull_epsilon_control3, 1, 4, 5);
  leg_entry = "freeze none,  0.5< chi2/ndf green <2, #mu = " + to_string(fitresult[0]).substr(0, 6) + ", #sigma= " + to_string(fitresult[1]).substr(0, 6);
  leg4->AddEntry(pull_epsilon_control3, leg_entry.c_str(), "l");
  pull_epsilon_control3->SetLineColor(kBlue);
  pull_epsilon_control3->Draw("E SAME");
  */
  leg4->Draw("");
