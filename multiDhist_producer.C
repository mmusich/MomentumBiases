	  ...
	  if(75.0<mll_reco && mll_reco<105.0){ //Cut in mll
		...
		if(pow(pow(GenPart_eta[k]-Muon_eta[i],2) + pow(GenPart_phi[k]-Muon_phi[i],2),0.5)<0.3){ 
		  ...
		  if(option==1){ 
		    ...
		    smear_pt = r->Gaus(GenPart_pt[k], width);
	    ...
	    //attention
	    mll_reco = r->Gaus(motherGen.M(), 0.03*motherGen.M());
...
int frame(){
  ...
  auto d2 = d1.Define("dxy_significance","dxy_significance(Muon_dxy, Muon_dxyErr)")
    ...
    .Define("pairs", "pairs(Muon_pt, Muon_charge, Muon_eta, Muon_phi, MuonisGood, Muon_dxy, Muon_dz, 0.105658, GenPart_status, GenPart_pdgId, GenPart_genPartIdxMother, GenPart_pt, GenPart_eta, GenPart_phi, 0)") //ATTENTION to last argument, the option for smeared pt is 1
    // muMass = 0.105658 GeV
  ...
  d4.Snapshot("Events", "snapshot_output.root", {"GenPart_status", "GenPart_pdgId", "GenPart_pt", "Muon_pt" ,"mll_reco", "mll_gen", "mll_diff"});
  ...
  */
  ...
  int nbinsmll_diff=15, nbinsmll=18, nbinseta=24, nbinspt=5;
  ...
  //ATTENTION change filename if options for pair is 1
  ...
  //ATTENTION do not change variable name mll_reco
  ...
  //f5->WriteObject(mDh_smear.GetPtr(), "multi_data_histo_smear");
  ...
  //ATTENTION do not change variable name mll_diff
